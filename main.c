...
void	hook_init(void *mlx, void *win, t_data *img, t_file *file, t_player player, t_data *buff)
{
	t_mlx	*mlxAll = malloc(sizeof(t_mlx));
	*mlxAll = (t_mlx){win, mlx, img, buff, file, player};
...
int	main(int argc, char **argv)
{
	...
	buff.img = mlx_new_image(mlx, 1920, 1080);
	buff.addr = mlx_get_data_addr(buff.img, &buff.bpp, &buff.line_len, &buff.endian);
	hook_init(mlx, mlx_win, &img, file, player, &buff);
	mlx_loop(mlx);
	free_all(file);
	return (0);
}
